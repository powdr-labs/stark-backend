...
Clean re-application of the [`autoopt-2026-04-12` run](https://github.com/powdr-labs/stark-backend/blob/autoopt-2026-04-12/autoopt-results/summary-report.md), adding the most promising changes. Each change is a single commit on top of the base `v2-powdr-07-04` branch. The goal is to give a human reviewer a focused list of what is worth upstreaming, ranked by per-commit payoff.

Commits are ordered by per-step impact on APC 300 (largest first) as much as dependencies allow. The only unavoidable inversion is at step 8, where the larger GPU Round 0 + overlap-logup change depends on step 7's `Round0ThreadBuffers` plumbing.

## Baseline setup: `VPMM_PAGE_SIZE=16777216`

The first change in the original recommended list was "Increase default VPMM page size from 2 MiB to 16 MiB" — a 10-line default-value change in `crates/cuda-common/src/memory_manager/vm_pool.rs`. That default is already overridable via the `VPMM_PAGE_SIZE` environment variable, so no code change is required; setting `VPMM_PAGE_SIZE=16777216` (16 MiB = 8x the RTX 4090's 2 MiB minimum granularity) reaches the env-var branch in `VpmmConfig::from_env()` and bypasses the built-in default.

The baseline (step 0) in this report is measured with that env var set. On its own, the env var is worth ~2,484 -> ~2,307 ms at APC 300 (1.08x, matching the original committed-default measurement of 2,315 ms), and ~2,176 -> ~1,795 ms at APC 0 (1.21x — the biggest APC-0 win in the whole run). All 8 optimization steps below build on that baseline.

## Summary

![STARK proving time excluding trace by iteration](stark_excl_trace_by_iteration.svg)

Measured STARK excl trace (from `spec.py`, in ms) after each commit, with `VPMM_PAGE_SIZE=16777216` set for every run. "vs base" compares to step 0; "vs prev" compares to the immediately preceding step. Each diff cell is `<absolute delta>ms (<factor>x)` — factor >1 is a decrease (improvement), "higher" marks a step-to-step regression.
...
Full per-phase numbers are in `results.csv`. APC 300 reaches **2.01x vs the tuned baseline** (1,148 ms vs 2,307 ms) and **2.16x vs the original untuned baseline** (1,148 ms vs 2,484 ms) once the env var benefit is rolled in.

The "vs prev" impact at APC 300 is `-392, -273, -184, -142, -66, -27, -10, -65` — strictly decreasing for the first seven transitions. The single uptick at step 8 is a structural consequence of step 7 being a prerequisite for step 8 (step 8 depends on `Round0ThreadBuffers` introduced by step 7).
...
Three steps collapse multiple original-run iterations into a single commit because the iterations cannot be applied or measured independently:

- **Steps 1 and 2 each fold in a constant bump.** The original multi-stream commits (iterations 4 and 5) landed with 4 streams; an immediately-following iteration 7 bumped both constants to 8 because `nsys` showed <2x effective parallelism at 4. Since 8 is strictly better on the RTX 4090 and the bump is a two-line change, steps 1 and 2 set 8 streams directly at introduction.

- **Step 7 bundles iterations 15 and 17** (round-robin work dispatch and pre-allocated Round 0 buffers). Iteration 17's buffer-pool design assumes iteration 15's round-robin dispatch is in place — they share the worker scope structure and cannot be reordered without code surgery.

- **Step 8 bundles iterations 21 and 22** (GPU-side Round 0 polynomial extraction and overlap logup precompute). Iteration 22's background-thread precompute reads from the `d_batch_array` that iteration 21 introduces, and the LogUp GKR portion of iteration 22's benefit depends on the CUDA memory pool state iteration 21's allocation pattern sets up. Landing them separately produces misleading per-step numbers; together they reliably deliver ~65 ms at APC 300 and ~16 ms at APC 100.

One change from the original recommended list is not included:

- **Batch MLE interpolation** (original iteration 11). Its phase-level signal on MLE Rounds is real (~-15 ms at APC 300 in the original run), but the STARK-total effect when applied on top of the other commits in this chain is within noise, and the commit costs 310 lines.
