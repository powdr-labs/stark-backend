...
// ============================================================================
// WARP-PER-TRACE KERNELS (for small traces: num_y <= 32, num_monomials <= 32)
// ============================================================================

// Each warp handles one trace. Thread lane = y_int.
// Each thread loops over ALL monomials for its y-value, producing a per-trace sum
// via warp reduction. Output is scattered to output_offsets[warp_global].
__global__ void warp_zerocheck_monomial_kernel(
    FpExt *__restrict__ output,
    const MonomialAirCtx *__restrict__ air_ctxs,
    const uint32_t *__restrict__ trace_ids,
    ...
    sum = sumcheck::warp_reduce_sum(sum);

    if (lane == 0) {
        output[output_offsets[warp_global] * num_x + x_int] = sum;
    }
}

// 4 warps per block = 128 threads
constexpr uint32_t WARP_KERNEL_WARPS_PER_BLOCK = 4;
constexpr uint32_t WARP_KERNEL_BLOCK_SIZE = WARP_KERNEL_WARPS_PER_BLOCK * WARP_SIZE;

extern "C" int _warp_zerocheck_monomial_batched(
    FpExt *output,
    const MonomialAirCtx *air_ctxs,
    const uint32_t *trace_ids,
    ...
    uint32_t num_blocks = div_ceil(num_traces, WARP_KERNEL_WARPS_PER_BLOCK);
    dim3 grid(num_blocks, num_x);
    dim3 block(WARP_KERNEL_BLOCK_SIZE);

    warp_zerocheck_monomial_kernel<<<grid, block>>>(
        output, air_ctxs, trace_ids, output_offsets, num_traces
    );
    return CHECK_KERNEL();
}

// Warp-per-trace logup monomial evaluation.
// Each warp evaluates both numerator and denominator monomials for one trace.
__global__ void warp_logup_monomial_kernel(
    FracExt *__restrict__ output,
    const LogupMonomialCommonCtx *__restrict__ common_ctxs,
    const LogupMonomialCtx *__restrict__ numer_ctxs,
    const LogupMonomialCtx *__restrict__ denom_ctxs,
    const uint32_t *__restrict__ trace_ids,
    const uint32_t *__restrict__ output_offsets,
    uint32_t num_traces
) {
    ...
    uint32_t trace_id = trace_ids[warp_global];
    LogupMonomialCommonCtx common = common_ctxs[trace_id];
    LogupMonomialCtx numer = numer_ctxs[trace_id];
    LogupMonomialCtx denom = denom_ctxs[trace_id];

    FpExt numer_sum(Fp::zero());
    FpExt denom_sum(Fp::zero());

    if (lane < common.num_y) {
        uint32_t y_int = lane;
        uint32_t height = num_x * common.num_y;
        uint32_t row = x_int * common.num_y + y_int;

        // Numerator monomials
        for (uint32_t m = 0; m < numer.num_monomials; ++m) {
            MonomialHeader hdr = numer.d_headers[m];
            FpExt monomial = numer.d_combinations[m];
            for (uint16_t v = 0; v < hdr.num_vars; ++v) {
                PackedVar var = numer.d_variables[hdr.var_offset + v];
                monomial *= eval_variable(var, row, common.eval_ctx, height);
            }
            numer_sum += monomial;
        }
        numer_sum *= common.d_eq_xi[y_int];

        // Denominator monomials
        for (uint32_t m = 0; m < denom.num_monomials; ++m) {
            MonomialHeader hdr = denom.d_headers[m];
            FpExt monomial = denom.d_combinations[m];
            for (uint16_t v = 0; v < hdr.num_vars; ++v) {
                PackedVar var = denom.d_variables[hdr.var_offset + v];
                monomial *= eval_variable(var, row, common.eval_ctx, height);
            }
            denom_sum += monomial;
        }
        // Add bus_term contribution (once per y_int)
        denom_sum += common.bus_term_sum;
        denom_sum *= common.d_eq_xi[y_int];
    }

    numer_sum = sumcheck::warp_reduce_sum(numer_sum);
    denom_sum = sumcheck::warp_reduce_sum(denom_sum);

    if (lane == 0) {
        uint32_t out_pos = output_offsets[warp_global];
        output[out_pos * num_x + x_int].p = numer_sum;
        output[out_pos * num_x + x_int].q = denom_sum;
    }
}

extern "C" int _warp_logup_monomial_batched(
    FracExt *output,
    const LogupMonomialCommonCtx *common_ctxs,
    const LogupMonomialCtx *numer_ctxs,
    const LogupMonomialCtx *denom_ctxs,
    ...
    warp_logup_monomial_kernel<<<grid, block>>>(
        output, common_ctxs, numer_ctxs, denom_ctxs,
        trace_ids, output_offsets, num_traces
    );
    return CHECK_KERNEL();
}

// Scatter-copy: copies blocks of `block_size` FpExt elements from contiguous src
// to scattered positions in dst. Used to merge block-path output into the shared
// output buffer.
__global__ void scatter_fpext_blocks_kernel(
    FpExt *__restrict__ dst,
    const FpExt *__restrict__ src,
    const uint32_t *__restrict__ dst_offsets,
    uint32_t num_items,
    uint32_t block_size
) {
    uint32_t item = blockIdx.x;
    uint32_t elem = blockIdx.y * blockDim.x + threadIdx.x;
    if (item >= num_items || elem >= block_size) return;
    dst[dst_offsets[item] * block_size + elem] = src[item * block_size + elem];
}

extern "C" int _scatter_fpext_blocks(
    FpExt *dst,
    const FpExt *src,
    ...
    if (num_items == 0 || block_size == 0) return 0;

    // Each block handles one item's worth of elements
    uint32_t threads = min(block_size, 256u);
    uint32_t y_blocks = div_ceil(block_size, threads);
    dim3 grid(num_items, y_blocks);
    dim3 blk(threads);

    scatter_fpext_blocks_kernel<<<grid, blk>>>(
        dst, src, dst_offsets, num_items, block_size
    );
    return CHECK_KERNEL();
}
