...
// Per-AIR metadata for batched coset-parallel evaluation.
...
__global__ void zerocheck_ntt_evaluate_constraints_coset_parallel_batched_kernel(
    ...
    if (threadIdx.x < skip_domain) {
        ...
        // Uses global blockIdx.x so blocks from different AIRs write to non-overlapping regions
...
extern "C" int _zerocheck_ntt_eval_constraints_batched(
    ...
    // Batched reduction: each AIR gets its own segment of the tmp_sums_buffer
    // Find max blocks per segment for reduction block size
