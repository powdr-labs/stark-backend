...
__global__ void batched_zerocheck_r0_coset_parallel_kernel(
    ...
    const Round0ZcCtx *__restrict__ d_zc_ctxs,
    uint32_t num_cosets,
    uint32_t skip_domain
) {
    extern __shared__ char smem[];
    ...
    NttEvalContext<1> eval_ctx{
        ctx.preprocessed, ctx.main_parts, ctx.public_values,
        inter_buffer, ntt_buffer, {omega_shift},
        skip_domain, ctx.height, buffer_stride, ctx.buffer_size, ntt_idx,
    };
    ...
    if (x_int < ctx.num_x) {
        ...
        FpExt constraint_sums[1];
        acc_constraints<1, NEEDS_SHMEM>(
            constraint_sums, eval_ctx, &is_first, &is_last, x_int,
            ctx.lambda_pows, ctx.d_rules, ctx.rules_len,
            ctx.d_used_nodes, ctx.used_nodes_len, ctx.lambda_len
        );
...
int launch_batched_zerocheck_r0(
    FpExt *tmp_sums_buffer, FpExt *output,
    const Round0BlockCtx *d_block_ctxs, const Round0ZcCtx *d_zc_ctxs,
    const uint32_t *d_air_offsets,
    uint32_t total_blocks, uint32_t num_airs,
    uint32_t num_cosets, uint32_t skip_domain, uint32_t block_x
) {
    ...
    batched_zerocheck_r0_coset_parallel_kernel<GLOBAL, NEEDS_SHMEM>
        <<<grid, block, shmem_bytes>>>(
            tmp_sums_buffer, d_block_ctxs, d_zc_ctxs, num_cosets, skip_domain
        );
    int err = CHECK_KERNEL();
    if (err != 0) return err;
...
extern "C" int _batched_zerocheck_r0_eval_constraints(
    FpExt *tmp_sums_buffer, FpExt *output,
    const void *d_block_ctxs, const void *d_zc_ctxs,
    const uint32_t *d_air_offsets,
    uint32_t total_blocks, uint32_t num_airs,
    uint32_t num_cosets, uint32_t skip_domain, uint32_t block_x
) {
    // Only NEEDS_SHMEM=false is instantiated. The benchmark uses l_skip=4
    // (skip_domain=16 <= WARP_SIZE), so needs_shmem is always false.
    // Pruning the NEEDS_SHMEM=true variant saves ~1MB of .cubin/.text.
