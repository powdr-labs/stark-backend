...
template <bool GLOBAL>
__global__ void batch_evaluate_interactions_gkr_kernel(
    const GkrBlockCtx *__restrict__ d_block_ctxs,
    const GkrInputCtx *__restrict__ d_air_ctxs
) {
    GkrBlockCtx bctx = d_block_ctxs[blockIdx.x];
    GkrInputCtx ctx = d_air_ctxs[bctx.air_idx];
...
extern "C" int _logup_gkr_input_eval_batched(
    bool is_global,
    const GkrBlockCtx *d_block_ctxs,
    const GkrInputCtx *d_air_ctxs,
    uint32_t num_blocks
) {
    if (num_blocks == 0) return 0;
    uint32_t threads_per_block = 256;
    if (is_global) {
        batch_evaluate_interactions_gkr_kernel<true><<<num_blocks, threads_per_block>>>(
            d_block_ctxs, d_air_ctxs
        );
    } else {
        batch_evaluate_interactions_gkr_kernel<false><<<num_blocks, threads_per_block>>>(
            d_block_ctxs, d_air_ctxs
        );
    }
    return CHECK_KERNEL();
