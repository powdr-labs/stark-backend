...
__global__ void batched_gkr_input_eval_kernel(
    const BatchGkrInputDesc *__restrict__ descs,
    const BatchBlockCtx *__restrict__ block_ctxs,
    FpExt *__restrict__ intermediates_base,
    const FpExt *__restrict__ d_challenges
) {
    BatchBlockCtx ctx = block_ctxs[blockIdx.x];
    BatchGkrInputDesc desc = descs[ctx.air_idx];

    uint32_t row = ctx.local_block_idx * blockDim.x + threadIdx.x;
    if (row >= desc.permutation_height) return;

    // Per-thread intermediates: interleaved layout with stride=height for coalesced warp access.
    // Adjacent threads access adjacent memory locations (same pattern as original per-AIR kernel).
    FpExt *intermediates_ptr = intermediates_base + desc.intermediates_offset + row;
    uint32_t intermediate_stride = desc.permutation_height;

    uint32_t rules_evaluated = 0;
    for (uint32_t used_idx = 0; used_idx < desc.used_nodes_len; used_idx++) {
        uint32_t node_idx = desc.d_used_nodes[used_idx];
        FpExt result(0);
        if (node_idx < rules_evaluated) {
            Rule rule = desc.d_rules[node_idx];
            RuleHeader header = decode_rule_header(rule);
            if (header.op == OP_VAR) {
                result = evaluate_dag_entry_gkr(
                    header.x, row,
                    desc.d_preprocessed, desc.d_main, desc.d_public_values,
                    d_challenges, intermediates_ptr, intermediate_stride,
                    desc.permutation_height
                );
            } else {
                uint32_t z_index = decode_z_index(rule);
                result = intermediates_ptr[z_index * intermediate_stride];
            ...
            for (; rules_evaluated <= node_idx; rules_evaluated++) {
                Rule rule = desc.d_rules[rules_evaluated];
                RuleHeader header = decode_rule_header(rule);
        ...
        uint32_t pair_idx = desc.d_pair_idxs[used_idx];
        size_t interaction_idx = pair_idx >> 1;
        size_t out_idx = (interaction_idx * desc.permutation_height + row) * 2;
        reinterpret_cast<FpExt *>(desc.d_fracs)[out_idx + (pair_idx & 1)] = result;
    }
}
...
extern "C" int _batched_gkr_input_eval(
    const BatchGkrInputDesc *descs,
    const BatchBlockCtx *block_ctxs,
    uint32_t total_blocks,
    FpExt *intermediates,
    const FpExt *d_challenges
) {
    if (total_blocks == 0) return 0;
    batched_gkr_input_eval_kernel<<<total_blocks, 256>>>(
        descs, block_ctxs, intermediates, d_challenges
    );
    return CHECK_KERNEL();
}
