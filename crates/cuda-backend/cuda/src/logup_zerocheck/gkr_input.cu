...
struct GkrInputCtx {
    ...
    FpExt *d_intermediates;        // pre-allocated (GLOBAL), or null
    FracExt *d_output;             // where to write this trace's results
};

struct GkrInputBlockCtx {
    uint32_t local_block_idx_x;
    uint32_t air_idx;
};

template <bool GLOBAL>
__global__ void evaluate_interactions_gkr_batched_kernel(
    const GkrInputBlockCtx *__restrict__ d_block_ctxs,
    const GkrInputCtx *__restrict__ d_trace_ctxs,
    const FpExt *__restrict__ d_challenges
) {
    GkrInputBlockCtx bctx = d_block_ctxs[blockIdx.x];
    GkrInputCtx ctx = d_trace_ctxs[bctx.air_idx];

    uint32_t task_offset = bctx.local_block_idx_x * blockDim.x + threadIdx.x;
    uint32_t task_stride = ctx.blocks_per_trace * blockDim.x;
    ...
    for (uint32_t j = 0; j < ctx.num_rows_per_tile; j++) {
        ...
        if (row < ctx.permutation_height) {
            ...
            for (uint32_t used_idx = 0; used_idx < ctx.used_nodes_len; used_idx++) {
                ...
                // Write to per-trace output buffer
                uint32_t pair_idx = ctx.d_pair_idxs[used_idx];
                size_t interaction_idx = pair_idx >> 1;
                size_t out_idx = (interaction_idx * ctx.permutation_height + row) * 2;
                reinterpret_cast<FpExt *>(ctx.d_output)[out_idx + (pair_idx & 1)] = result;
            }
        }
    }
}

extern "C" int _gkr_input_eval_batched(
    bool is_global,
    const GkrInputBlockCtx *d_block_ctxs,
    const GkrInputCtx *d_trace_ctxs,
    const FpExt *d_challenges,
    ...
    dim3 block(256);
    if (is_global) {
        evaluate_interactions_gkr_batched_kernel<true><<<grid, block>>>(
            d_block_ctxs, d_trace_ctxs, d_challenges
        );
    } else {
        evaluate_interactions_gkr_batched_kernel<false><<<grid, block>>>(
            d_block_ctxs, d_trace_ctxs, d_challenges
        );
    }
    return CHECK_KERNEL();
