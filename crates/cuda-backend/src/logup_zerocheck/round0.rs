...
/// Per-AIR metadata for batched zerocheck coset-parallel evaluation (GLOBAL=false).
...
/// Per-AIR metadata for batched zerocheck coset-parallel evaluation (GLOBAL=true).
/// Must match the C struct `ZerocheckBatchMetaGlobal` in zerocheck_round0.cu exactly.
#[repr(C)]
#[derive(Clone, Copy)]
...
/// Per-AIR metadata for batched logup coset-parallel evaluation (GLOBAL=false).
...
/// Per-AIR metadata for batched logup coset-parallel evaluation (GLOBAL=true).
/// Must match the C struct `LogupBatchMetaGlobal` in logup_round0.cu exactly.
#[repr(C)]
#[derive(Clone, Copy)]
pub(crate) struct LogupBatchMetaGlobal {
    pub selectors_cube: *const F,
    pub preprocessed: *const F,
    pub main_parts: *const *const F,
    pub eq_cube: *const EF,
...
// SAFETY: All pointers refer to device memory managed by DeviceBuffer kept alive
// until stream sync.
unsafe impl Send for LogupBatchMetaGlobal {}
