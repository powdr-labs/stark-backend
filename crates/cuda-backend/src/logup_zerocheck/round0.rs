...
/// Pre-computed interaction data for a single AIR, ready for kernel launch.
...
/// Prepare interaction data for a single AIR (CPU-heavy work + H2D uploads).
/// This extracts the DAG construction, rule encoding, and weight computation
/// that was previously done inside `evaluate_round0_interactions_gpu`.
pub fn prepare_round0_interactions(
    symbolic: &SymbolicConstraints<F>,
    eq_3bs: &[EF],
    beta_pows: &[EF],
) -> Result<InteractionRound0Prep, Round0EvalError> {
...
/// Launch the zerocheck constraints kernel without allocating intermediates or syncing.
/// Caller must provide pre-allocated buffers. Does NOT perform D2H copies.
#[allow(clippy::too_many_arguments)]
pub fn launch_round0_constraints_kernel<HS: GpuHashScheme>(
...
/// Launch the logup interactions kernel without allocating intermediates or syncing.
/// Caller must provide pre-allocated buffers and pre-computed interaction prep.
#[allow(clippy::too_many_arguments)]
pub fn launch_round0_interactions_kernel(
    ...
    unsafe {
        logup_bary_eval_interactions_round0(
            ...
            eq_cube,
            public_values,
            &prep.d_numer_weights,
            &prep.d_denom_weights,
            prep.denom_sum_init,
            &prep.d_rules,
            prep.buffer_size,
            intermediates,
...
/// Compute the required zerocheck intermediates buffer size for a given AIR.
pub fn zc_intermediates_size(
    buffer_size: u32,
    skip_domain: u32,
    num_x: u32,
    num_cosets: u32,
...
/// Compute the required zerocheck temp_sums buffer size for a given AIR.
pub fn zc_temp_sums_size(
    buffer_size: u32,
    skip_domain: u32,
    num_x: u32,
    num_cosets: u32,
...
/// Compute the required logup intermediates buffer size for a given AIR.
pub fn logup_intermediates_size(
    buffer_size: u32,
    skip_domain: u32,
    num_x: u32,
    num_cosets: u32,
...
/// Compute the required logup temp_sums buffer size for a given AIR.
pub fn logup_temp_sums_size(
    buffer_size: u32,
    skip_domain: u32,
    num_x: u32,
    num_cosets: u32,
